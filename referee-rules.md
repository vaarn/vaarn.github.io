# Referee Rules and Content Generators
...
## Monsters
__Hit Dice / Hit Points:__ All monster hit dice (__HD__) are d8s unless otherwise specified. To get the monster's hit points, just multiply the number of hit dice they have by 4 (or 5 if you're feeling mean.)  

__ARMOUR:__ A monster's Armour score gives the number a PC's attack role must equal or exceed to damage them.  

__Example:__ A typical 4 HD monster would have an attack bonus of +4 and a defence of 14 in all of its abilities by default, unless modified by the referee.  

__Damage:__ This indicates the number of attacks a monster can make per round, and the damage dice rolled for each.  
